...
To deregister from **every** detected CLI at once (Claude Code, Codex, opencode, Grok Build, Cursor), use `sema setup --uninstall` — `sema init` targets only one client and has no `--opencode` flag.
...
```bash
# 1. Deregister from every detected AI client (Claude Code, Codex, opencode, Grok Build, Cursor)
